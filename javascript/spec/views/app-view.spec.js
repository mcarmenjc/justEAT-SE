describe ('app.AppView', function(){
	...
	describe('when searching for restaurants', function(){
		describe('and outcode is set', function(){
			...
			it ('should fetch collection with outcode se19', function(){
				expect(view.Restaurants.fetch).toHaveBeenCalledWith('se19');
			});
		});
	...
	describe('# showServerError', function(){
		beforeEach(function(){
			view.Restaurants.trigger('fetchError');
		});
		it('should be defined', function(){
			expect(view.showServerError).toBeDefined();
		});
		it('should show error in the view', function(){
			expect(view.$error).toContainText('We are having problems, please try again later');
		});
	});
});